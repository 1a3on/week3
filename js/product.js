...
//   ]
  ...
  const app = Vue.createApp({
    ...
    methods:{
        checkLogin(){  //登入驗證                                   
            const api = rootPath + 'api/user/check';
            axios.post(api)
                .then(res=>{
        ...
        getData(){
            const api = `${rootPath}api/${apiPath}/admin/products`;
        ...
        updateProduct(){       
            ...
            let api = `${rootPath}api/${apiPath}/admin/product`;
        ...
        deleteProduct(){
          const api = `${rootPath}api/${apiPath}/admin/product/${this.tempProduct.id}`
          axios.delete(api)
            .then(res=>{
              alert(res);
    ...
    mounted(){
        productModal = new bootstrap.Modal(document.getElementById("productModal"),{keyborad:false});
